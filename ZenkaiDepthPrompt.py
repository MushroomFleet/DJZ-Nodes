...
class ZenkaiDepthPrompt:
    @classmethod
    def INPUT_TYPES(cls):
        # Determine the path to the depthprompts folder relative to this file
        depthprompts_folder = os.path.join(os.path.dirname(__file__), "depthprompts")
        
        # Create the folder if it doesn't exist
        if not os.path.exists(depthprompts_folder):
            os.makedirs(depthprompts_folder)
            
        # List subdirectories in the depthprompts folder
        subfolders = [d for d in os.listdir(depthprompts_folder) if os.path.isdir(os.path.join(depthprompts_folder, d))]
        
        # If there are no subfolders, provide a placeholder
        if not subfolders:
            subfolders = ["default"]
            # Create a default folder
            default_folder = os.path.join(depthprompts_folder, "default")
    ...
    def contains_whitelist_term(self, text, whitelist_terms):
        if not whitelist_terms:
            return True  # If no whitelist terms, all prompts pass
    ...
    def load_depth_prompt(self, prompt_folder, seed, mode="sequential", num_images=1, whitelist=""):
        # Build absolute path to the selected subfolder under depthprompts
        folder_path = os.path.join(os.path.dirname(__file__), "depthprompts", prompt_folder)
        ...
        if whitelist_terms:
            valid_pairs = [(img, text) for img, text in valid_pairs 
                           if self.contains_whitelist_term(text, whitelist_terms)]
            
            if not valid_pairs:
                return (torch.zeros(1, 3, 64, 64), "No valid pairs matching whitelist criteria")
...
NODE_CLASS_MAPPINGS = {
    "ZenkaiDepthPrompt": ZenkaiDepthPrompt
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "ZenkaiDepthPrompt": "Zenkai Depth Prompt"
}
